/* Solução Móveis — Modelo D (híbrido) · interações */
...
document.querySelectorAll('.sec, .stats').forEach(el => { el.classList.add('reveal'); io.observe(el); });
...
const countUp = (el) => {
  ...
  const tick = (t) => { if (!start) start = t; const p = Math.min((t - start) / dur, 1);
    el.textContent = (suf === '+' ? '+' : '') + fmt(Math.floor(p * target)) + (suf === '+' ? '' : suf);
